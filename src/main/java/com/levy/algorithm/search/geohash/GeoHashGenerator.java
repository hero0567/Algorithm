...
public class GeoHashGenerator {
    ...
    public GeoHashGenerator(double lat, double lng) {
     ...
     */
    private List<String> getGeoHashBase32For9() {
    ...
    private List<String> getGeoHashBase32For9(LocationBean loc) {
        double leftLat = loc.getLat() - minLat;
        double rightLat = loc.getLat() + minLat;
        double upLng = loc.getLng() - minLng;
        double downLng = loc.getLng() + minLng;
        List<String> base32For9 = new ArrayList<String>();
        //左侧从上到下 3个
        String leftUp = getGeoHashBase32(leftLat, upLng);
        if (!(leftUp == null || "".equals(leftUp))) {
        ...
        String leftMid = getGeoHashBase32(leftLat, loc.getLng());
        if (!(leftMid == null || "".equals(leftMid))) {
            base32For9.add(leftMid);
        }
        String leftDown = getGeoHashBase32(leftLat, downLng);
        ...
        //中间从上到下 3个
        String midUp = getGeoHashBase32(loc.getLat(), upLng);
        if (!(midUp == null || "".equals(midUp))) {
            base32For9.add(midUp);
        }
        String midMid = getGeoHashBase32(loc.getLat(), loc.getLng());
        if (!(midMid == null || "".equals(midMid))) {
            base32For9.add(midMid);
        }
        String midDown = getGeoHashBase32(loc.getLat(), downLng);
        if (!(midDown == null || "".equals(midDown))) {
            base32For9.add(midDown);
        }
        //右侧从上到下 3个
        ...
        String rightMid = getGeoHashBase32(rightLat, loc.getLng());
        if (!(rightMid == null || "".equals(rightMid))) {
            base32For9.add(rightMid);
        }
        String rightDown = getGeoHashBase32(rightLat, downLng);
    ...
    public List<String> getGeoHashBase32For9(double lat, double lng) {
        LocationBean location = new LocationBean(lat, lng);
        return getGeoHashBase32For9(location);
    }
    ...
    public static void main(String[] args) {
        ...
        GeoHashGenerator g = new GeoHashGenerator(40.222012, 116.248283);
        g.sethashLength(12);
        System.out.println("当前坐标：" + g.getGeoHashBase32());

        GeoHashGenerator g1 = new GeoHashGenerator();
        g1.sethashLength(12);
        System.out.println("当前坐标：" + g1.getGeoHashBase32(40.222012, 116.248283));

        for (String str : g.getGeoHashBase32For9()) {
            System.out.println(str);
        }

        for (String str : g1.getGeoHashBase32For9(40.222012, 116.248283)) {
            System.out.println(str);
        }
    }
}